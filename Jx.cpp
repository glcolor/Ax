...
void RegisterTimerLibrary(JetContext* context)
{
	...
	{
		Timer* t = new Timer();
		return context->CreateNewUserData(t, TimerPrototype);
	};

	TimerPrototype = context->AddPrototype("Timer").m_Object;//new JetObject(context);
	...
	while (true)
	{
		...
		else if (strcmp(command2, "test") == 0 && arg[0] == 0)
		{
			...
			try
			{
				...
				try
				{
					ValueRef meta = tcontext.AddPrototype("Test");//ok, lets give me a type
